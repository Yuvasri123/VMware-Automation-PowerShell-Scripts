<#
.Synopsis
 Monthly Capacity Report for HCI Environment.

.DESCRIPTION
 The Script will perform checks and provide both Monthly and 24 Hours data for clusterwise report, ESXIHostwise report. 
 This report will contain vSAN utilization of cluster, CPU and Memory Average.
...
.OUTPUT
 The HTML report will be generated in 'HCI-Monthly Capacity report.html' file stored in the powershell solution path. 
 The CSV file will be generated in 'Clusterlevel_Utilization.csv' , 'ESXIHost_Utilization.csv' file stored in the powershell solution path
...
$reportname = "$path\HCI-Monthly Capacity report.html"
...
if (($file -match "Clusterlevel_Utilization") -or ($file -match "ESXIHost_Utilization"))
{
...
foreach ($vCen in $InputvcenterFile.vCenter)
{
  ...
  $csv_file2 = @()
    ...
    try
    {     
        $statucpu = Get-VMhost -Location $clus | Get-Stat -stat cpu.usage.average -start $start -Finish $stop -MaxSamples ([int]::MaxValue) | Measure-Object -Property value -Average -Maximum -Minimum -ErrorAction Stop      
        ...
        $memoryusage = Get-VMhost -Location $clus | Get-Stat -Stat mem.usage.average -start $start -Finish $stop -MaxSamples ([int]::MaxValue) | Measure-Object Value -Average -Maximum -Minimum
        ...
        if ($vSanUsed) {          
        $used_capacityy=[math]::round( ($VsanUsed * 100 ) / $VsanSpaceUse.CapacityGB)
    ...
    $csv1 = [psCustomObject] [Ordered]@{
                        ...
                        CPUAvg                        = $CPUAvg
                        MemoryAvg                     = $MEMAvg
                        "VSANUtilization(UsedSpace)"  = $utilized_percentage
                        "VSAN(FreeSpace)"             = $free_percentage
                    } 
                    
                    $csv_file1 += $csv1
    }
     
    $csv_file1 | Export-Csv -Path $clustercsv -NoTypeInformation -Append
    ...
    $tabledata1 += '<thead><tr>'
    foreach ($head in "vCenter","Cluster","CPUAvg","MemoryAvg","VSAN Utilization(UsedSpace)","VSAN(FreeSpace)"){
    ...
    foreach($row in $table1){
        ...
        $tabledata1 += $row.UsedSpace
        $tabledata1 += $row.FreeSpace
        $tabledata1 += '</tr>'
       }
    $tabledata1 += '</tbody>'
    $tabledata1 += '</table>'
...
{
$cl = $onecl.name
$hosts = get-cluster -name $cl | Get-VMHost
$cl
    foreach($vmHost1 in $hosts)
    {
        ...
        try{
            ...
            $statcpu =  Get-Stat -Entity $vmHost -Stat cpu.usage.average -start $start -Finish $stop -MaxSamples ($MaxCount) | Measure-Object -Property value -Average -Maximum -Minimum -ErrorAction Stop
            $memusage = Get-Stat -Entity $vmHost -Stat mem.usage.average -start $start -Finish $stop -MaxSamples ($MaxCount) |Measure-Object Value -Average -Maximum -Minimum 
    ...
    $csv2 = [psCustomObject] [Ordered]@{
     ...
     $csv_file2 | Export-Csv -Path $hostcsv -NoTypeInformation -Append
...
<title>HCI- Monthly Capacity Report</title>


<nav class='navbar sticky-top navbar-dark' style='background-color: #5F249F;'>
<center><a class='navbar-brand'><h2 style=color:white> Monthly Capacity Report for HCI Environment</h2></a></center>
...
$tabledata1
<center><h3>Hostwise Report</h3></center>
$tabledata2
...
Send-MailMessage -From $From -to $To -Subject "Monthly Capacity Report for HCI Environment" -Cc $Cc -Body "Attachment of output HTML Report and CSV files" -BodyAsHtml -SmtpServer $SMTPServer -port $SMTPPort -Attachments $reportname,$clustercsv, $hostcsv
